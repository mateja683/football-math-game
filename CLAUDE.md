...
- `readStart()`/`readNext()`/`readMark(correct)` — reading game (screen `reading`, state `RD`). Shows a word from `READ_WORDS` (3–4 letters) in large print — deliberately **not** spoken aloud, unlike maths questions, since the whole point is the child decoding it themselves. `recognizer` (Web Speech API, feature-detected from `SpeechRecognition`/`webkitSpeechRecognition`) can auto-confirm a correct read via the 🎤 button, but only ever auto-*confirms* (never auto-marks wrong — a mismatch just asks the grown-up to check); the `#readRight`/`#readAgain` buttons are the reliable fallback/override and work with or without recognizer support.
