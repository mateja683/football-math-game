...
- `Pong` — IIFE module for the two-player canvas game, in two variants: `Pong.start(after, 'classic')` is the original keeper-vs-keeper match, `Pong.start(after, 'foosball')` adds a rod of three outfield players per side (`rods()`, `drawRods()`) that slide with the same finger as that side's keeper. Outfield rods only block shots heading at their *own* goal, so a rod can never deflect the ball into an own goal; they shrink with the keepers. `Pong.freeze()` cancels the animation loop without running the after-callback (used by the session lock). Bottom keeper = child's team, top keeper = `other(S.team)`. First to 3 (`WIN`), no rematch — winning always ends the session (`pback` → `stop()` → `after()`) and awards that variant's trophy. Pointer events map each touch to its half of the pitch; arrow-button mode uses the same `keys` set as the keyboard (A/D top, ←/→ bottom). `askExit()` shows an in-canvas "Leave the match?" confirm on the ✕ button mid-play instead of exiting instantly. Keepers shrink (`padWEff`, floored at `MIN_PAD_SCALE`) the longer a rally stays at `maxSpeed()` without a goal, via `fullSpeedStreak` (reset in `serve()`).
- `Mini` — IIFE shell the three tap-based prize games plug into, with its own `#mini` overlay and `#mc` canvas. It owns the canvas sizing, the rAF loop, the intro message, the ✕ exit confirm and `finish(title, sub)` (which awards the trophy and shows the Done button), and hands games drawing helpers (`pitch`, `ballAt`, `netGoal`, `bar`, `text`, `dot`, `line`). A game is a plain object with `title`, `intro`, `setup(W,H)`, `update(dt)`, `draw()`, optional `down/move/up(x,y)` and `resize(W,H)`. **Games are held still until the intro message clears** — otherwise gravity/spawns run before the child can play.
- `Penalty` / `Keepy` / `Scramble` — the three `Mini` games. Penalties is tap-to-aim (not swipe — easier at four), 5 shots, keeper dives with `SAVE_CHANCE` ≈ ⅓. Keepy-uppies has a generous tap radius (`r * 2.6`) and a gentle centring force so the ball doesn't park against a wall; the personal best lives in `S.keepyBest`. Scramble runs `TIME` seconds with a spawn rate that ramps up; balls that get past only tick a counter, they never end the game.
- `PRIZE_GAMES` / `openPicker(after, spend)` / `usePrize(after)` / `playPrizeGame(gm)` — one list drives the picker grid, the trophy cabinet and the launching, so adding a game in one place wires up all three. The prize is only spent when a game is actually chosen, so backing out of the picker costs nothing; the grown-up-gated "Prize games" button on the maths screen opens the same picker with `spend` false. `awardTrophy(key)` bumps `S.trophies[key]` and is called on *finishing* a game, not on leaving early.
